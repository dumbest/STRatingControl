...
open class STRatingControl: UIView {
    ...
    override open func layoutSubviews() {
        ...
        padding = max(0.0, (bounds.width - CGFloat(width)) / 2.0)
        ...
        for (index, button) in ratingButtons.enumerated() {
            buttonFrame.origin.x = CGFloat(index * (buttonSize + spacing)) + padding
    ...
    func handleStarTouches(_ touches: Set<UITouch>, withEvent event: UIEvent?) {
        if let touch = touches.first {
            ...
            if (position.x - padding) > -50 && (position.x - padding) < CGFloat(width + 50) {
    ...
    func ratingButtonSelected(_ position: CGPoint) {
        for (index, button) in ratingButtons.enumerated() {
            if (position.x + padding) > button.frame.minX + padding {
                self.rating = index + 1
